...
// Make About the only active button
...
function buildAndShowAboutHTML (aboutHtml) {
  switchAboutToActive();
  var max = 5;
  var stars = getRandomNumber();
  
  for(var count = 1; count <= max; count++){
    if(count <= stars){
      aboutHtml = insertProperty(aboutHtml, "class" + count, "fa fa-star");
    } else {
      aboutHtml = insertProperty(aboutHtml, "class" + count, "fa fa-star-o");
    }
  }
  aboutHtml = insertProperty(aboutHtml, "total", stars);
